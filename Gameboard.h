...
// previous tetromino being ìlockedî (where its blocks are copied to the gameboard).
// 
...
// - A 2D array uses row - major ordering by default ñ which means that the first index
//      is the row and the second is the column.eg: grid[row][col]. However it will be
//      easier for us to think of this grid in terms of XY coordinates. Hereís the 
//      catchÖ X represents columns and Y represents rows.  (I know this seems counter-
...
public:	
   ...
   int getContent(int x, int y) const ;
	...
	void setContent(const std::vector<Point>& pt, int content);
	...
	bool areAllLocsEmpty(const std::vector<Point>& pt) const;
...
private:  // This is commented out to allow us to test. 
	...
	// - return: nothing
	void copyRowIntoRow(int rowSource, int rowTarget);
